...
def main():
    ...
    for func in (main_hg, main_git):
        ...
        raise RuntimeError()
    url, ver = url
    main_parse_and_gen(drepo, url, ver)
...
def main_parse_and_gen(dname, url, ver):
    info("use url as: %s" % url)
    for parser in (parse_github, parse_bitbucket):
        url_src, url_files = parser(url, ver)
        if url_src:
            break
    else:
        raise RuntimeError(
    ...
    fname = os.path.join(dname, "android", "RhinoForAndroid",
                         "src", "com", "googlecode",
                         "rhinoforandroid", "RhinoUrls.java")
 ...
 */
package com.googlecode.rhino%sforandroid;

public class Rhino%sUrls {
...
def main_git(drepo):                                        # {{{1
    ...
    for sec in cfg.sections():
        ...
        if (not url.startswith("git://") and
            ...
            continue
        txt = subprocess.check_output("git tag", shell=True)
        latest = ["rhino-6.1.2"]
        for line in txt.splitlines():
            if "rhino" not in line:
                continue
            if line in latest:
                eror("%s is already in versions" % line)
                continue
            latest.append(line.strip())
        latest = sorted(latest, reverse=True)
        latest = latest[0]
        return url, latest
...
def parse_github(url, ver):                                 # {{{1
    ...
    url_src = ("https://github.com" + up.path +
               "/blob/lang-rhino/rhino/")
    return url_src, url_files


def parse_bitbucket(url, ver):                              # {{{1
    up = urlparse(url)
